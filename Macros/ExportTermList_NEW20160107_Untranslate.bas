...
Set trnlst = prj.TransLists(i)		'取得某个翻译列表
	 ...
	 With wb.Sheets(trnlst.Language.LangCode)
		...
		For j = 1 To trnlst.StringCount 				'遍历字串列表
			...
			If trnlst.String(j).State(pslStateTranslated) = Flase And trnlst.String(j).State(pslStateReadOnly) = Flase Then'处理need to black string
					...
					.Range("g" & CStr(row+1)) = s
						If trnlst.String(j).State(pslStateReadOnly) Then
						s = "Locked"
          					ElseIf trnlst.String(j).State(pslStateReview) Then
						s = "Review"
          					ElseIf Not trnlst.String(j).State(pslStateTranslated) Then
						s = "Not Translated"
          					ElseIf trnlst.String(j).State(pslStateBookmark) Then
						s = "Bookmark"
          					End If
