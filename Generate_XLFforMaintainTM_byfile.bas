...
''Export the translated [en->target] stringlist as bilingual xlf file for Maintaining TM.
...
Sub Main
	...
	Begin Dialog UserDialog 930,140,"Generate XLF for Maintain TM",.BrowseFolderPath ' %GRID:10,7,1,1
	...
	PSL.Output "********** Start - Generate XLF for Maintain TM **********"
	...
	For Each tranlst In proj.TransLists
		...
		End If
	Next
	PSL.Output "********** End - Generate XLF for Maintain TM **********"
	MsgBox "DONE!", vbOkOnly, "Generate XLF for Maintain TM"
End Sub
...
Sub GenerateXLF(tslst As PslTransList,pathOut As String)
	...
	For idxstr = 1 To tslst.StringCount Step 1
		...
		If transtr.State(pslStateReadOnly) = False And transtr.State(pslStateLocked) = False And transtr.State(pslStateHidden) = False And transtr.State(pslStateTranslated) = True Then
			...
			targetNd.setAttribute("state", "final")
			targetNd.Text = transtr.Text
